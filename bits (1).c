...
int bitOr(int x, int y) {
  /*
   * Using DeMorgan's law ~ means complement
   */
  return ~(~x & ~y);
}
/* 
...
/* 
 * 0 binary: 00000000
 * 0 complement: 11111111
 * 0 2-complement system: -1
 */
  return ~0x00;
}
/* 
...
int allEvenBits(int x) {
   /*Mask: 01010101 this is where all even positions are 1
    *Shifted by 2 bits and added again to make it 32 bits
    *x & mask to get where the x and mask match
    *Using XOR so if it is all the same then the inverse will change the 0 to a 1
    */
...
int anyOddBit(int x) {
   /*Mask: 10101010
    *Shifting one byte each time while using XOR: 0^1=1
    *(we have 0's in odd-numbered bits in our mask so our mask will be 01010101)
    *return x & mask flipped twice
    */
  int mask= (0xAA);
  mask = (mask<< 8)^mask;
  mask = (mask << 16)^mask;
...
int byteSwap(int x, int n, int m) {
    /*shift entire byte left by 3
    *We shift 0xFF by a and use OR with 0xFF shift by b then save the complement
    *Shift everything again by a, and use & with 0xFF to get the new bytes
...
int addOK(int x, int y) {
    /*use XOR with the sign of x and y, to return 1 if they are the same
    *Compare sign of x with the sum of x and y
    *double inverse then return (DeMorgan's Law?)
    */
...
int replaceByte(int x, int n, int c) {
    /*Storing the entire byte n within y by shifting 3
    *Mask of 1's that shifts by a byte
    *Shift bytes and replace with the byte c (bytes that are added) by using mask and OR
    */
    int y= n << 3;
    int mask= 0xFF << y;
    int shift = c << y;
...
int satAdd(int x, int y) {
    /*Sum the two values
    *Check for overflow by using XOR sum with x & comparing with XOR sum of y then getting the sign of integer
